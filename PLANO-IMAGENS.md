...
- [x] **Homolog**: deploy servidor homolog + build `assembleHomologDebug` no stick Intelbras de teste (`192.168.31.182`, kiosk reprovisionado pra `com.paitv.homolog` via `deploy/kiosk-intelbras.ps1 -Flavor homolog -Aplicar`).
  ...
  - [x] *Opcional, não crítico*: APK antigo de verdade contra servidor novo — já coberto pela validação do formato do array `videos` legado via curl; não repetido por falta de um APK antigo instalado à mão. **Validado em 2026-08-05**: recompilado o código-fonte exato de produção (branch `main`, commit `6381c81`, anterior à feature de imagens) com o flavor `homolog`, instalado no stick Intelbras de homolog (TIHomolog). Sync completou sem erro ("Sync concluído. 1/1 vídeo(s) em cache."), app antigo ignorou a imagem da playlist mista (protegido pelo array `videos` legado do servidor) e reproduziu o vídeo normalmente, sem crash.
